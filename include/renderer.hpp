...
namespace kanso {
	...
	class renderer {
		public:
			...
			virtual void bind_texture(uint shader, const std::string& type, uint id, uint& diffuse_nr, uint& specular_nr,
			                         uint& height_nr, uint& normal_nr, uint& number, uint index) = 0;
